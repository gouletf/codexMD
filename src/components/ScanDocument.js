...
//import { uploadFile, uploadImageData } from "../firebase";
...
export default function ScanDocument() {
	...
	useEffect(() => {
		async function getMedia() {
			...
			video.srcObject = await window.navigator.mediaDevices.getUserMedia({
			    audio: false,
			    video: {
				width: 9999,
				height: 9999,
	...
	function handleVideoClick() {
		...
		globalContext.uploadFile(imageData)

		// Promise.resolve(uploadImageData(imageData)).then(function(snapshot) {
	...
	<section id="bottom-navigation" className="block fixed inset-x-0 bottom-0 z-10 bg-white bg-opacity-25 shadow">
		<div id="tabs" className="flex justify-between text-white">
			<div className="w-full justify-left inline-block text-left pt-3 pb-1 pl-4">
				<button className="focus:text-teal-500 hover:text-teal-500">
					<svg width="45" height="45" viewBox="0 0 50 50" className="inline-block">
			...
			<div className="w-full justify-center inline-block text-center pt-2 pb-1">
				<button onClick={handleVideoClick} id="imgButton"  className="focus:text-teal-500 hover:text-teal-500">
					<svg width="45" height="45" viewBox="0 0 50 50" className="inline-block">
						<g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
							<g transform="translate(5.000000, 5.000000)">
								<circle stroke="currentColor" strokeWidth="2" cx="21" cy="21" r="22.5"></circle>
							</g>
						</g>
					</svg>
				</button>
